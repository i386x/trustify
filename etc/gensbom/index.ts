...
function warning(message: string): void {
  console.error(
    `${colors.YELLOW}WARNING[${process.argv[1]}]: ${message}${colors.RESET}`,
  );
}

function error(message: string): never {
  console.error(
    `${colors.RED}ERROR[${process.argv[1]}]: ${message}${colors.RESET}`,
  );
  process.exit(1);
}
...
async function processImages(inputFile: string): Promise<void> {
  ...
  if (!existsSync(dockerConfig)) {
    warning(
      `Docker configuration with credentials (\`${dockerConfig}\`) not present`,
    );
  ...
  if (!TPA_AUTH_TOKEN) {
    ...
    warning(
      `Authorized communication with ${TPA_SERVICE_URL} will not be possible`,
    );
  } else {
  ...
  for await (const line of rl) {
    ...
    try {
      execSync(
        `"${SYFT_COMMAND}" -v scan "${image}" -o "cyclonedx-json=${sbomPath}"`,
        {
          stdio: "inherit",
          shell: true,
        },
